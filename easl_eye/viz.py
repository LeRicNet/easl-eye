	...
	for i, image in enumerate(images):
		...
		if aligned_data.in_roi_filter_x[i] and aligned_data.in_roi_filter_y[i]:
			if aligned_data.segment_class[i] == 'Fixation':
				...
				bounding_box = eye.basic_expand(edges, center_point, buffer=1)
				bbox = patches.Rectangle((10, 10), 200, 300, edgecolor='green', facecolor=(0,0,0,0), lw=2)
				plt.gca().add_patch(bbox)
				# print('Center Point: {}; Bounding Box: {}'.format(center_point, bounding_box))
				frames.append([image])
				# frames.append([image, show_box(bounding_box, ax=ax)])
				# bbox.remove()
		else:
			color = 'red'
			overlay = plt.scatter(aligned_data.right_roi_filter_scaled_x[i], 
								  aligned_data.right_roi_filter_scaled_y[i], color=color)
...
def show_box(box, ax=None):
    ...
    return patches.Rectangle((x0, y0), w, h, edgecolor='green', facecolor=(0,0,0,0), lw=2)
